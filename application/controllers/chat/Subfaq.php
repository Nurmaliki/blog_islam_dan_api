...
class Subfaq extends CI_Controller {
	public $contak="Faq";
	public $title="Flayer";
	...
	public function  datapage($tipe){
		...
		$sql_total = $this->inc->count_all('content_faq', ["id_parent"=>$tipe]);
        
		$sql_data = $this->inc->filter($search, $limit, $start, $order_field, $order_ascdesc, 'content_faq', false,
										array('(@row_number := @row_number +1)  AS no','id','judul','content','like','unlike'), array('id,judul,content'), false, ["id_parent"=>$tipe],false); 
        
		$sql_filter = $this->inc->count_filter($search, 'content_faq', false, array('id,judul,content'), false,  ["id_parent"=>$tipe]); 
        $data=array();
		foreach($sql_data as $xr){
			...
			$data[]=array($xr['no'],$xr['judul'],$page,$xr['content']
			);
		}
	...
	function Created($tipe,$id=""){
		$result_parent=$this->inc->ResultAllby_ID("content_faq",["id"=>$tipe])[0];
		// print_r($result_parent);
		// die();
			...
			$html='<form id="Faqfrom" enctype="multipart/form-data"><div class="row">
				<div class="col-md-12">
					<div class="form-group">
						...
						<label for="exampleInputEmail1">Sub FAQ '.$result_parent->judul.' Title</label>
						<input type="text" class="form-control" name="judul" id="exampleInputEmail1" placeholder="Content '.$result_parent->judul.'">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label for="exampleInputEmail1">Sub FAQ '.$result_parent->judul.'</label>
			...
			$html='<form id="Faqfrom" enctype="multipart/form-data"><div class="row">
				<div class="col-md-12">
					<div class="form-group">
						...
						<input type="hidden" value="'.$tipe.'" name="tipe">
						<label for="exampleInputEmail1">Sub FAQ '.$result_parent->judul.' Title</label>
						<input type="text" class="form-control" name="judul" id="exampleInputEmail1" placeholder="Sub FAQ '.$result_parent->judul.'">
					</div>
					<div class="form-group">
	...
	function EditData($id){
		...
		$result_parent=$this->inc->ResultAllby_ID("content_faq",["id"=>$result[0]->id_parent])[0];
		if(count($result)>0){
			$result=$result[0];
			...
			$html='<form id="Faqfrom" enctype="multipart/form-data"><div class="row">
				<div class="col-md-12">
					...
					<input type="hidden" value="Edit" name="store">
						...
						<input type="hidden" value="'.$result->id_parent.'" name="tipe">
						<label for="exampleInputEmail1">Sub FAQ '.$result_parent->judul.' Title</label>
						<input type="text" class="form-control" name="judul" id="exampleInputEmail1" placeholder="Sub FAQ '.$result_parent->judul.'" value="'.$result->judul.'">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1">Upload Icon</label>
						<input type="file" name="file" class="form-control" id="exampleInputEmail1">
	...
	function DeleteData($id){
		...
		if(count($result)>0){
			...
			redirect('page-content-faq/'.$result->id_parent);
		}
	...
	function Store(){
		...
		if(isset($_FILES["file"]["name"]) && $_FILES["file"]["name"] != ''){	
			...
			$datas["tipe"]='faq';	
			$datas["id_parent"]=$this->input->post('tipe');	
