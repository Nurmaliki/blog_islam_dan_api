...
class Status extends CI_Controller {
	...
	public function  datapage($tipe){
		...
		$sql_total = $this->inc->count_all('content_status', ["tipe"=>$tipe]);
        
		$sql_data = $this->inc->filter($search, $limit, $start, $order_field, $order_ascdesc, 'content_status', false,
										array('(@row_number := @row_number +1)  AS no','id','judul','content','like','unlike'), array('id,judul,content'), false, ["tipe"=>$tipe],false); 
        
		$sql_filter = $this->inc->count_filter($search, 'content_status', false, array('id,judul,content'), false,  ["tipe"=>$tipe]); 
        $data=array();
		foreach($sql_data as $xr){
			$urlink=base_url()."page-content/".$xr['id'];
			$urlink_delete=base_url()."StatusDelete/".$xr['id']; 

			if ($tipe == 'artikel' || $tipe == 'diliput-oleh' ||  $tipe == 'inspirasi' ||  $tipe == 'fitur') {
				$page='<a href="'.$urlink.'"  class="btn-sm btn-rounded btn-outline-success btn-sm" target="_blank">View<a/> &nbsp;<span type="button" id="'.$xr['id'].'" class="btn-sm btn-rounded btn-outline-info btn-sm" onclick="return EditPage(this.id)" >Edit</span>  &nbsp;<a href="'.$urlink_delete.'"  id="'.$xr['id'].'" class="btn-sm btn-rounded btn-outline-danger btn-sm" onclick="return confirm(`Are you sure you want to delete?`)" >Delete</a>';

			} else {
				$page='<a href="'.$urlink.'"  class="btn-sm btn-rounded btn-outline-success btn-sm" target="_blank">View<a/> &nbsp;<span type="button" id="'.$xr['id'].'" class="btn-sm btn-rounded btn-outline-info btn-sm" onclick="return EditPage(this.id)" >Edit</span> ';
	...
	function EditData($id){
		$result=$this->inc->ResultAllby_ID("content_status",["id"=>$id]);
		if(count($result)>0){
			$result=$result[0];
	...
	function DeleteData($id){
		...
		if(count($result)>0){
			$result=$result[0];
			$delete=$this->inc->DeleteDataALl(["id"=>$id],'content_status'); 	
	...
	function Store(){
		...
		if($submit=="Edit"){
			...
			$page=$this->inc->ResultAllby_ID("content_status",["id"=>$id]);
			if(count($page)>0){
		...
		if($submit=="Save"){
			$save=$this->inc->IncSave_Data('content_status',$datas); 	
		}else{
		
			$save=$this->inc->IncUpdate_Data('content_status',["id"=>$id],$datas); 	
		}
	...
	function page($id){
		$page=$this->inc->ResultAllby_ID("content_status",["id"=>$id]);
		$data["content"]='page-status';
		$data["title"]=$this->title;
		$data["result"]=$page;

		$this->load->view('content/page-status',$data);
