...
class Artikel extends CI_Controller
{
	...
	public function index()
	{
		...
		$data["artikel"] = $this->dbload->resulSqlQuery("SELECT * FROM `content_status` where tipe='artikel' ORDER BY create_date DESC	LIMIT " . $config['per_page'] . " OFFSET " . $from);
		$data["links"] = $this->pagination->create_links();


		// print_r($data);

		$this->load->view('web/newWeb/artikel', $data);
	}
	...
	public function detail_by_title($url)
	{
		...
		$data["artikel_terkait"] = $this->dbload->resulSqlQuery("SELECT * FROM `content_status` where tipe='artikel' ORDER BY create_date DESC	LIMIT 2");
		$data["artikel"] = $this->dbload->resulSqlQuery("SELECT * FROM `content_status` where tipe='artikel' AND judul = '$url'")[0];
		$this->load->view('web/newWeb/artikel_detail', $data);
	}
}
