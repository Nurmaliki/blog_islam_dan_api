...
class JOBVeritrans {
    ...
    {
        JOBVeritrans::$serverKey = $params['server_key'];
        JOBVeritrans::$isProduction = $params['production'];
    }
    ...
    */
  	...
  	{
    	return JOBVeritrans::$isProduction ?
        	JOBVeritrans::PRODUCTION_BASE_URL : JOBVeritrans::SANDBOX_BASE_URL;
  	}
    ...
    {	
	    ...
	    if (count(JOBVeritrans::$curlOptions)) {
	      // We need to combine headers manually, because it's array and it will no be merged
	      if (JOBVeritrans::$curlOptions[CURLOPT_HTTPHEADER]) {
	        $mergedHeders = array_merge($curl_options[CURLOPT_HTTPHEADER], JOBVeritrans::$curlOptions[CURLOPT_HTTPHEADER]);
	        $headerOptions = array( CURLOPT_HTTPHEADER => $mergedHeders );
	      } else {
	        $mergedHeders = array();
	      }

	      $curl_options = array_replace_recursive($curl_options, JOBVeritrans::$curlOptions, $headerOptions);
	    }
    ...
    {	
    	...
    	$result = JOBVeritrans::post(
        JOBVeritrans::getBaseUrl() . '/charge',
        JOBVeritrans::$serverKey,
        $payloads);

        return $result->redirect_url;


    	//$url = JOBVeritrans::getBaseUrl();
    	//return JOBVeritrans::$serverKey.JOBVeritrans::getBaseUrl() . '/charge' ;
    }
    ...
    */
	public static function status($id)
 	{
    	return JOBVeritrans::get(
        	JOBVeritrans::getBaseUrl() . '/' . $id . '/status',
        	JOBVeritrans::$serverKey,
        	false);
  	}
  	...
  	{
    	return JOBVeritrans::post(
        	JOBVeritrans::getBaseUrl() . '/' . $id . '/approve',
        	JOBVeritrans::$serverKey,
        	false)->status_code;
  	}
  	...
  	{
    	return JOBVeritrans::post(
        	JOBVeritrans::getBaseUrl() . '/' . $id . '/cancel',
        	JOBVeritrans::$serverKey,
        	false)->status_code;
  	}
    ...
    */
  	...
  	{
    	return JOBVeritrans::post(
        	JOBVeritrans::getBaseUrl() . '/' . $id . '/expire',
        	JOBVeritrans::$serverKey,
        	false);
  	}
